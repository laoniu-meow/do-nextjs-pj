...
>(({ onSaveDisabledChange, onUploadDisabledChange, onMessageChange }, ref) => {
  ...
  const loadSupplierData = React.useCallback(async () => {
    ...
    try {
      ...
      const stagingRes = await fetchWithAuth(
        "/api/admin/products/suppliers/staging",
        {
          cache: "no-store",
        }
      );
  ...
  const handleSave = React.useCallback(async () => {
    try {
      ...
      const res = await fetchWithAuth("/api/admin/products/suppliers/staging", {
        method: "PUT",
        headers: {
  ...
  const handleUpload = React.useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await fetchWithAuth("/api/admin/products/suppliers/publish", {
        method: "POST",
      });
