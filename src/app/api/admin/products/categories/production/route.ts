...
// POST: Move staging categories to production (same pattern as tax and supplier)
  ...
  try {
    ...
    if (stagingData.length === 0) {
      ...
      return NextResponse.json({
        success: true,
        data: [],
        message: 'Production categories cleared (no staging data to upload)',
      });
    }
    ...
    return NextResponse.json({
      success: true,
      data: result,
      message: `${result.length} category(ies) moved to production successfully`,
    });
  } catch (error) {
    console.error('Error uploading categories to production:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to upload categories to production' }, 
      { status: 500 }
    );
  }
}
